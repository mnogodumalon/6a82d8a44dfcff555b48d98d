...
export default function HundePage() {
  ...
  const [records, setRecords] = useState<Hunde[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Hunde | null>(null);
  ...
  async function handleCreate(fields: Hunde['fields']) {
  ...
  async function handleUpdate(fields: Hunde['fields']) {
    ...
    await LivingAppsService.updateHundeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('hunde') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('hunde') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('impfstatus')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hunde', 'impfstatus')}
                  {sortKey === 'impfstatus' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('fuetterungshinweise')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hunde', 'fuetterungshinweise')}
                  {sortKey === 'fuetterungshinweise' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('medikamente_besonderheiten')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hunde', 'medikamente_besonderheiten')}
                  {sortKey === 'medikamente_besonderheiten' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/hunde/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('hunde', 'geschlecht', record.fields.geschlecht?.key) ?? record.fields.geschlecht?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.gewicht_kg ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('hunde', 'impfstatus', record.fields.impfstatus?.key) ?? record.fields.impfstatus?.label ?? '—'}</span></TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.fuetterungshinweise ?? '—'}</span></TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.medikamente_besonderheiten ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('hunde') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
