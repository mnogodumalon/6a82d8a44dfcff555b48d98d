/**
 * HundeDialog — pre-generated create/edit dialog for Hunde.
 *
 ...
 * HundeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Hunde['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<HundeDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Hunde, Besitzer, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Hunde';
...
/** Widened prefill type for HundeDialog.defaultValues — see file header. */
export type HundeDialogDefaults = Omit<Hunde['fields'], 'geschlecht' | 'impfstatus'> & {
    geschlecht?: LookupValue | string;
    impfstatus?: LookupValue | string;
  };

interface HundeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Hunde['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: HundeDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  geschlecht: LOOKUP_OPTIONS['hunde']?.['geschlecht'] ?? [],
  impfstatus: LOOKUP_OPTIONS['hunde']?.['impfstatus'] ?? [],
};
...
export function HundeDialog({ open, onClose, onSubmit, defaultValues, recordId, besitzerList, enablePhotoScan = true, enablePhotoLocation = true }: HundeDialogProps) {
  const [fields, setFields] = useState<Partial<Hunde['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['name', 'besitzer'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Hunde['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'hunde');
      await onSubmit(clean as Hunde['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Name des Hundes\n  "rasse": string | null, // Rasse\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "maennlich" | "weiblich") mapping: maennlich=Männlich, weiblich=Weiblich\n  "gewicht_kg": number | null, // Gewicht (kg)\n  "impfstatus": LookupValue | null, // Impfstatus (select one key: "vollstaendig" | "teilweise" | "nicht_geimpft" | "unbekannt") mapping: vollstaendig=Vollständig geimpft, teilweise=Teilweise geimpft, nicht_geimpft=Nicht geimpft, unbekannt=Unbekannt\n  "fuetterungshinweise": string | null, // Fütterungshinweise\n  "medikamente_besonderheiten": string | null, // Medikamente & Besonderheiten\n  "besitzer": string | null, // Display name from Besitzer (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Hunde['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('hunde') })
    : t('new_entity', { entity: appLabel('hunde') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="geburtsdatum"
          placeholder="Wann wurde der Hund geboren?"
          mode="date"
    ...
    'geschlecht': (
      <div key="geschlecht" className="space-y-1.5">
        <Label htmlFor="geschlecht">{fieldLabel('hunde', 'geschlecht')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('hunde', 'geschlecht', 'maennlich') ?? 'Männlich'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hunde', 'geschlecht', 'weiblich') ?? 'Weiblich'}
          </button>
        </div>
      </div>
    ),
    'gewicht_kg': (
      <div key="gewicht_kg" className="space-y-1.5">
        <Label htmlFor="gewicht_kg">{fieldLabel('hunde', 'gewicht_kg')}</Label>
        <Input
          id="gewicht_kg"
          type="number"
          step="any"
    ...
    'impfstatus': (
      <div key="impfstatus" className="space-y-1.5">
        <Label htmlFor="impfstatus">{fieldLabel('hunde', 'impfstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'vollstaendig'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'vollstaendig' ? undefined : 'vollstaendig') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'vollstaendig'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('hunde', 'impfstatus', 'vollstaendig') ?? 'Vollständig geimpft'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'teilweise'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'teilweise' ? undefined : 'teilweise') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'teilweise'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('hunde', 'impfstatus', 'teilweise') ?? 'Teilweise geimpft'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hunde', 'impfstatus', 'nicht_geimpft') ?? 'Nicht geimpft'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'unbekannt'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'unbekannt' ? undefined : 'unbekannt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'unbekannt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('hunde', 'impfstatus', 'unbekannt') ?? 'Unbekannt'}
          </button>
        </div>
      </div>
    ),
    'fuetterungshinweise': (
      <div key="fuetterungshinweise" className="space-y-1.5">
        <Label htmlFor="fuetterungshinweise">{fieldLabel('hunde', 'fuetterungshinweise')}</Label>
        <Textarea
          id="fuetterungshinweise"
          placeholder="Fütterungszeiten, Menge, besondere Diäten, Allergien..."
    ...
    'medikamente_besonderheiten': (
      <div key="medikamente_besonderheiten" className="space-y-1.5">
        <Label htmlFor="medikamente_besonderheiten">{fieldLabel('hunde', 'medikamente_besonderheiten')}</Label>
        <Textarea
          id="medikamente_besonderheiten"
          placeholder="Medikamente, Dosierung, Verhalten, Angststörungen..."
          value={fields.medikamente_besonderheiten ?? ''}
          onChange={e => setFields(f => ({ ...f, medikamente_besonderheiten: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">{fieldLabel('hunde', 'foto')}</Label>
        {fields.foto ? (
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name des Hundes", "rasse": "Rasse", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "gewicht_kg": "Gewicht (kg)", "impfstatus": "Impfstatus", "fuetterungshinweise": "Fütterungshinweise", "medikamente_besonderheiten": "Medikamente & Besonderheiten", "foto": "Foto des Hundes", "besitzer": "Besitzer"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"besitzer": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.HUNDE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
