    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('aufenthalte', 'anreisedatum')}</Label>
            <p className="text-sm">{formatDate(record.fields.anreisedatum)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('aufenthalte', 'abreisedatum')}</Label>
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('aufenthalte', 'herkunft')}</Label>
            <Badge variant="secondary">{lookupLabel('aufenthalte', 'herkunft', record.fields.herkunft?.key) ?? record.fields.herkunft?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('aufenthalte', 'preis_euro')}</Label>
