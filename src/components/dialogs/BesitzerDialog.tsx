/**
 * BesitzerDialog — pre-generated create/edit dialog for Besitzer.
 *
 ...
 * BesitzerDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Besitzer['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<BesitzerDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Besitzer';
...
/** Widened prefill type for BesitzerDialog.defaultValues — see file header. */
export type BesitzerDialogDefaults = Besitzer['fields'];
...
interface BesitzerDialogProps {
  ...
  defaultValues?: BesitzerDialogDefaults;
...
export function BesitzerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: BesitzerDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Besitzer['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'besitzer');
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('besitzer') })
    : t('new_entity', { entity: appLabel('besitzer') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('besitzer', 'telefon')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="telefon"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('besitzer', 'email')}</Label>
        <Input
          id="email"
          type="email"
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besondere Wünsche, allergische Reaktionen oder Kontaktinformationen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BESITZER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
